...
final class CompanionDetailBottomView: BaseView {
    ...
    override func setUI() {
        ...
        peopleStackView.addArrangedSubviews(peopleIconImageView, peopleImageStackView,
                                            overflowCountLabel, peopleStatusLabel, peopleSpacerView)
    ...
    private func configureOverflowCount(_ participantCount: Int) {
        ...
        peopleStackView.setCustomSpacing(overflowCount > 0 ? 0 : 6, after: peopleImageStackView)
    ...
    func configure(state: CompanionDetailState) {
        ...
        peopleImageStackView.configure(withImageURLs: Array(state.participantImageURLs.prefix(profileAvatarCount)))
        ...
        case .immediate(let expirationTime):
            let message = expirationTime == "곧" ? "이 글은 곧 사라져요!" : "이 글은 \(expirationTime)에 사라져요!"
